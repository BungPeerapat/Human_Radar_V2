...
//
// Usage:
//   Log::info(TAG_MQTT, "Connected to %s:%d", host, port);
//   Log::warn(TAG_WIFI, "Signal low: %ddBm", rssi);
//   Log::error(TAG_SENSOR, "UART timeout");
//
// Legacy (backward compatible):
//   Log::info("Simple message");  → tag defaults to SYSTEM
// ============================================================================

// Forward declaration — MQTT log publish callback
// Set by mqtt_client after init to avoid circular dependency
...
inline const LogEntry& getBufferEntry(uint16_t index) {
    ...
    if (_bufCount < LOG_BUFFER_SIZE) {
        ...
        start = _bufHead;  // oldest is at head (about to be overwritten)
    }
...
inline void log(LogLevel level, const char* tag, const char* fmt, va_list args) {
    ...
    // Forward to MQTT (if enabled and level meets threshold)
...
// ---- Print radar frame (legacy + new tagged version) ----
inline void printFrame(const RadarFrame& frame, uint32_t frameCount = 0, uint32_t errorCount = 0) {
    // Compact one-line format for MQTT-friendly logging
    ...
    // Also print detailed format to Serial only (not to MQTT)
    Serial.println("----------------------------------------");
